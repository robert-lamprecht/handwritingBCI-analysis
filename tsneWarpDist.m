%This script was taken from fwillett/handwritingBCI
...
function warpDist = tsneWarpDist(d1, d2Mat, nTimeBinsPerTrial)
    ...
    affineWarps = linspace(0.7,1.42,15); %15 total possible alpha values to test 
    
    %infer the number of neural dimensions per data point
    nNeuralDim = length(d1)/nTimeBinsPerTrial; 
    
    %reshape d1 into a T x D matrix
    d1 = reshape(d1,nTimeBinsPerTrial,nNeuralDim); %this is the X variable in the equation
    
    %eDist represnts the euclidean distance between d1 and all rows of d2
    %for each alpha. 
    eDist = zeros(size(d2Mat,1), length(affineWarps)); %creating matrix to hold the difference between X and Y for all possible alpha values 
    ...
    for a=1:length(affineWarps)
        ...
        for rowIdx=1:size(d2Mat,1)
            ...
            d2 = d2Mat(rowIdx,:); %D2 represents the Y value in the equation
